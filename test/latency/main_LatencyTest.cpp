...
const option::Descriptor usage[] = {
    ...
    { RELIABILITY,     0, "r", "reliability",     Arg::Required,
      "  -r <arg>,    --reliability=<arg>   设置可靠性 (\"reliable\"/\"besteffort\")."},
    { SAMPLES,         0, "s", "samples",         Arg::Numeric,
      "  -s <num>,    --samples=<num>       样本数量." },
    { SEED,            0, "",  "seed",            Arg::Numeric,
      "               --seed=<num>          计算domain和topic的种子.(seed:种子,Partition分区机制,将Topic中的数据分区成多个子集,以便更好地管理数据,seed可以作为Topic的一部分,用于初始化数据域中某些变量或参数的值,以便在DDS系统中生成具有一定规律或随机性的数据值。例如,在DDS中可以通过随机数种子计算数据的哈希值等。)" },
    { HOSTNAME,        0, "",  "hostname",        Arg::None,
      "               --hostname            将hostname附加到topic." },
    { XML_FILE,        0, "",  "xml",             Arg::String,
      "               --xml                 XML配置文件." },
    { FORCED_DOMAIN,   0, "",  "domain",          Arg::Numeric,  "               --domain              RTPS Domain." },
    { FILE_R,        0, "f", "file",            Arg::Required,
      "  -f <arg>,  --file=<arg>             要从中读取有效负载需求的文件(csv表格文件)." },
    { DYNAMIC_TYPES,   0, "",  "dynamic_types",   Arg::None,
      "               --dynamic_types       使用动态数据types." },
    { DATA_SHARING,  0, "d", "data_sharing",    Arg::Enabler,
      "               --data_sharing=[on|off]             显式启用/禁用数据共享功能." },
    { DATA_LOAN,        0, "l", "data_loans",            Arg::None,
      "               --data_loans          使用借贷样本 API." },
    { SHARED_MEMORY,    0, "", "shared_memory", Arg::Enabler,
      "               --shared_memory=[on|off]             显式启用/禁用共享内存传输。" },
#if HAVE_SECURITY
    {
        USE_SECURITY,    0, "",  "security",        Arg::Required,
        "               --security <arg>      启用/禁用 安全性组件 (\"true\"/\"false\")."
    },
    { CERTS_PATH,      0, "",  "certs",           Arg::Required,
      "               --certs <arg>         (安全性组件使用的)证书所在的路径." },
#endif // if HAVE_SECURITY
    {
        UNKNOWN_OPT,     0, "",  "",                Arg::None,     "\nPublisher/Both 选项:"
    },
    { SUBSCRIBERS,     0, "n", "subscribers",     Arg::Numeric,
      "  -n <num>,    --subscribers=<arg>   subscribers的数量." },
    { EXPORT_CSV,      0, "",  "export_csv",      Arg::None,
      "               --export_csv          用于导出 CSV 文件的标志." },
    { EXPORT_RAW_DATA, 0, "",  "export_raw_data", Arg::String,
      "               --export_raw_data     将所有原始数据导出为 CSV 的文件名." },
    { EXPORT_PREFIX,   0, "",  "export_prefix",   Arg::String,
      "               --export_prefix       CSV 文件的文件前缀." },
    { UNKNOWN_OPT,     0, "",  "",                Arg::None,     "\nSubscriber 选项:"},
    { ECHO_OPT,        0, "e", "echo",            Arg::Required,
      "  -e <arg>,    --echo=<arg>          Echo模式 (\"true\"/\"false\")." },
    { 0, 0, 0, 0, 0, 0 }
};
        ...
        std::vector<uint32_t>& demands)
{
    ...
    std::cout << "读取需求文件: " << demands_file << std::endl;
    std::string DELIM = ";";
    if (!fi.is_open())
    {
        std::cout << "不能打开需求文件: " << demands_file << " , 关闭中." << std::endl;
        return false;
    }
    ...
    while (std::getline(fi, line))
    {
        ...
        while (more)
        {
            ...
            if (payload < 16)
            {
                std::cout << "有效负载必须是大于或等于 16 的正数" << std::endl;
                return false;
            }
        ...
        char** argv)
{
    ...
    if (use_security)
    {
        ...
        {
            EPROSIMA_LOG_ERROR(LatencyTest, "进程内通信不受安全性组件支持");
            return 1;
        }
        else if (Arg::EnablerValue::ON == data_sharing)
        {
            EPROSIMA_LOG_ERROR(LatencyTest, "RTPS 加密不支持共享样本API");
            return 1;
        }
    }
#endif // if HAVE_SECURITY
    ...
    {
        EPROSIMA_LOG_ERROR(LatencyTest, "动态类型不支持共享样本API");
        return 1;
    }
    ...
    {
        std::cout << "使用 " << subscribers << " 个subscribers和 " << samples << " 个样本"
                  << std::endl;
    ...
    {
        std::cout << "使用 " << subscribers << " 个subscribers 和 " << samples <<
            " 个样本进行进程间通信" << std::endl;
    ...
    if (return_code == 0)
    {
        std::cout << C_GREEN << "测试完成。一切正常" << C_DEF << std::endl;
    }
    else
    {
        std::cout << C_RED << "出了一些错误" << C_DEF << std::endl;
    }
