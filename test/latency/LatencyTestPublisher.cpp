...
LatencyTestPublisher::~LatencyTestPublisher()
{
    ...
    if (dynamic_types_)
    {
        destroy_data_endpoints();
            ...
            || !latency_data_type_)
    {
        EPROSIMA_LOG_ERROR(LATENCYPUBLISHER, "注销 DATA type 和/或 移除 endpoints 时出错");
    }
        ...
        LatencyDataSizes& latency_data_sizes)
{
    ...
    *output_files_[AVERAGE_INDEX] << std::endl;
    ...
    {
        EPROSIMA_LOG_ERROR(LATENCYPUBLISHER, "注册 COMMAND type 时出错");
        return false;
    }
    ...
    {
        EPROSIMA_LOG_ERROR(LATENCYPUBLISHER, "创建 PUBLISHER 时出错");
        return false;
    }
    ...
    {
        EPROSIMA_LOG_ERROR(LATENCYPUBLISHER, "创建 SUBSCRIBER 时出错");
        return false;
    }
    ...
    std::cout << "开销 " << overhead_time_.count() << " 微秒" << std::endl;
    ...
    if (raw_data_file_ != "")
    {
        ...
        data_file << "样本,负载 [字节],延迟 [微秒]" << std::endl;
    }
        ...
        DataReader* reader)
{
    ...
    {
        EPROSIMA_LOG_INFO(LatencyTest, "读取command message时出错");
    }
}
        ...
        DataReader* reader)
{
    ...
    if (pub->data_loans_)
    {
        ...
        {
            EPROSIMA_LOG_ERROR(LatencyTest, "读取用户回显借出的测试数据时出问题了");
            return;
        }
    }
    else
    {
        ...
        if (reader->take_next_sample(
                ...
                || !info.valid_data)
        {
            EPROSIMA_LOG_ERROR(LatencyTest, "读取Subscriber回显的测试数据时出问题了");
            return;
        }
    }
    ...
    bool notify = false;
    {
        ...
        {
            EPROSIMA_LOG_INFO(LatencyTest, "收到的回显消息不是预期的消息");
        }
        else
        {
