...
const option::Descriptor usage[] = {
    ...
    { SUBSCRIBERS,     0, "n", "subscribers",     Arg::Numeric,
      "  -n <num>,    --subscribers=<arg>   订阅者数量." },
    { TIME,          0, "t", "time",            Arg::Numeric,
      "  -t <seconds>,  --time=<seconds>             测试时间（秒）." },
    { RECOVERY_TIME, 0, "",  "recovery_time",   Arg::Numeric,
      "             --recovery_time=<milliseconds>    如果发送时间短于 ，则其余<recovery_time>时间休眠" },
    { RECOVERIES,    0, "",  "recoveries_file", Arg::String,
      "             --recoveries_file=<num>  包含恢复时间的 CSV 文件" },
    { DEMAND,        0, "d", "demand",          Arg::Numeric,
      "  -d <num>,  --demand=<num>           块中发送的样本数（默认值：10000）。" },
    { MSG_SIZE,      0, "s", "msg_size",        Arg::Numeric,
      "  -s <num>,  --msg_size=<num>         消息的大小（以字节为单位）（默认值：1024）。" },
    { FILE_R,        0, "f", "file",            Arg::Required,
      "  -f <arg>,  --file=<arg>             要从中读取有效负载要求的文件。" },
    { EXPORT_CSV,    0, "",  "export_csv",      Arg::String,
      "             --export_csv             用于导出 CVS 文件的标志。" },
    { UNKNOWN_OPT,   0, "",   "",               Arg::None,
      "\nNote:\n如果未提供需求或msg_size则使用.csv文件.\n"},
    { 0, 0, 0, 0, 0, 0 }
};
        ...
        char** argv)
{
    ...
    {
        EPROSIMA_LOG_ERROR(ThroughputTest, "进程内交付不支持安全性组件启用");
        return 1;
    }
    else if ( Arg::EnablerValue::ON == data_sharing && use_security )
    {
        EPROSIMA_LOG_ERROR(ThroughputTest, "RTPS 加密不支持共享示例 API");
        return 1;
    }
#endif // if HAVE_SECURITY
    ...
    {
        EPROSIMA_LOG_ERROR(ThroughputTest, "动态类型不支持共享示例 API");
        return 1;
    }
    ...
    {
        std::cout << "启动吞吐量测试发布者代理" << std::endl;
    ...
    {
        std::cout << "启动吞吐量测试订阅者代理" << std::endl;
    ...
    {
        std::cout << "启动吞吐量测试共享进程模式" << std::endl;
    ...
    {
        std::cout << C_RED << "发生了错误" << C_DEF << std::endl;
    }
