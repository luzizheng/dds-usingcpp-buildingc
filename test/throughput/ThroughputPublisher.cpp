        ...
        const eprosima::fastdds::dds::PublicationMatchedStatus& info)
{
    ...
    {
        EPROSIMA_LOG_INFO(THROUGHPUTPUBLISHER, C_RED << "Pub: DATA Pub 已匹配 "
        ...
        const SubscriptionMatchedStatus& info)
{
    if (1 == info.current_count)
    {
        EPROSIMA_LOG_INFO(THROUGHPUTPUBLISHER, C_RED << "Pub: COMMAND Sub 已匹配 "
        ...
        const eprosima::fastdds::dds::PublicationMatchedStatus& info)
{
    if (1 == info.current_count)
    {
        EPROSIMA_LOG_INFO(THROUGHPUTPUBLISHER, C_RED << "Pub: COMMAND Pub 已匹配 "
                                                     << info.total_count << "/" << throughput_publisher_.subscribers_ * 2 <<
                C_DEF);
    }
        ...
        int forced_domain)
{
    ...
    std::cout << "发布者的时钟访问开销：" << t_overhead_.count() << " us"  << std::endl;
        ...
        uint32_t subscribers)
{
    ...
    if (export_csv_ != "")
    {
        ...
        data_file << "负载 [字节],需求 [样本/突发],恢复时间[ms],发送 [样本],发布时间 [微秒],"
                  << "发布样本速率 [每个/秒],发布吞吐量 [Mb/s],接收 [样本],"
                  << "丢失 [样本],订阅时间 [us],订阅样本速率 [每个/秒],"
                  << "订阅吞吐量 [Mb/s]" << std::endl;
        data_file.flush();
        data_file.close();
    }

    std::cout << "Pub: 等待命令发现" << std::endl;
    {
    ...
    for (auto sit = demand_payload_.begin(); sit != demand_payload_.end(); ++sit)
    {
        ...
        if (dw_qos.history().kind == KEEP_LAST_HISTORY_QOS)
        {
            ...
            {
                EPROSIMA_LOG_WARNING(THROUGHPUTPUBLISHER, "将历史记录深度设置为 " << max_demand);
        ...
        else
        {
            ...
            {
                EPROSIMA_LOG_WARNING(THROUGHPUTPUBLISHER, "将资源限制最大样本数设置为 " << max_demand);
                dw_qos.resource_limits().max_samples = max_demand;
            }
        }
        ...
        if (dynamic_types_)
        {
            ...
            {
                EPROSIMA_LOG_ERROR(THROUGHPUTPUBLISHER, "迭代失败：无法创建动态数据");
                return;
            }
        ...
        else
        {
            ...
            {
                EPROSIMA_LOG_ERROR(THROUGHPUTPUBLISHER, "准备用于测试的静态类型和终结点时出错");
                test_failure = true;
                break;
            }
        ...
        else
        {
            ...
            {
                EPROSIMA_LOG_ERROR(THROUGHPUTPUBLISHER, "删除用于测试的静态类型和终结点时出错");
                test_failure = true;
                break;
            }
        ...
        uint32_t msg_size)
{
    ...
    while ( !results_error && num_results_received < subscribers_ )
    {
                ...
                && info.valid_data)
        {
            ...
            {
                std::cout << "测试预期结果，停止" << std::endl;
                results_error = true;
            }
        }
        else
        {
            std::cout << "读取结果时出现问题;" << std::endl;
            results_error = true;
        }
    }
...
bool ThroughputPublisher::load_demands_payload()
{
    ...
    std::cout << "读取需求文件: " << demands_file_ << std::endl;
    std::string DELIM = ";";
    if (!fi.is_open())
    {
    ...
    while (std::getline(fi, line))
    {
        ...
        while (more)
        {
            ...
            if (first)
            {
                ...
                {
                    std::cout << "最小有效负载为 16 字节" << std::endl;
                    return false;
                }
                payload_ -= overhead;
                first = false;
    ...
    std::cout << "使用此有效负载/需求执行测试:" << std::endl;
    for (auto sit = demand_payload_.begin(); sit != demand_payload_.end(); ++sit)
    {
