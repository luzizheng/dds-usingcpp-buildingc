        ...
        const SubscriptionMatchedStatus& info)
{
    ...
    else
    {
        std::cout << info.current_count_change
                  << " 不是 SubscriptionMatchedStatus 当前计数更改的有效值" << std::endl;
    }
}
        ...
        DataReader* reader)
{
    ...
    while ((reader->take_next_sample(&hello_, &info) == ReturnCode_t::RETCODE_OK) && !is_stopped())
    {
        if (info.instance_state == ALIVE_INSTANCE_STATE)
        {
            samples_++;
            // 在此处打印您的结构数据
            std::cout << "消息 " << hello_.message().data() << " " << hello_.index() << " 已接收到" << std::endl;
        ...
        uint32_t samples)
{
    ...
    if (samples > 0)
    {
        std::cout << "Subscriber 正在运行，直到 " << samples <<
            " 个样本被接收. 请按 CTRL+C 随时停止Subscriber." << std::endl;
    }
    else
    {
        std::cout << "Subscriber 正在运行. 请按 CTRL+C 随时停止Subscriber." << std::endl;
    }
    signal(SIGINT, [](int signum)
            {
                std::cout << "接收到SIGINT信号, 停止Subscriber执行." << std::endl;
