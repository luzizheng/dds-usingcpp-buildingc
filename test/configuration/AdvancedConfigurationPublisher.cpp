        ...
        const eprosima::fastdds::dds::PublicationMatchedStatus& info)
{
    ...
    else
    {
        std::cout << info.current_count_change
                  << " 不是PublicationMatchedStatus当前计数更改的有效值" << std::endl;
    }
}
        ...
        uint32_t sleep)
{
    ...
    if (samples == 0)
    {
        std::cout << "Publisher正在运行. 请按 Ctrl+C 随时停止Publisher." << std::endl;
    }
    else
    {
        std::cout << "Publisher正在运行 " << samples <<
            " 个样本. 请按 Ctrl+C 随时停止Publisher." << std::endl;
    }
    signal(SIGINT, [](int signum)
            {
                std::cout << "收到SIGINT, 停止Publisher执行." << std::endl;
